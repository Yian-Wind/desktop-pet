...
const api: PetApi = {
  ...
  movePet: (x: number, y: number): void => ipcRenderer.send(IPC.PET_MOVE, x, y),
  ...
  setCoatOn: (packId: string, on: boolean): Promise<AppConfig> => ipcRenderer.invoke(IPC.COAT_SET, packId, on),
